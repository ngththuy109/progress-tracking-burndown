...
5. **Hint "chỉnh thứ tự Sub-phase ở đâu".** PM phản hồi: chọn Phase có nhiều Sub-phase nhưng "chưa tìm thấy chỗ setting thứ tự hiển thị sub-phase trên signboard". Chỗ chỉnh vốn TỒN TẠI — chính là thứ tự **② Phase list** ở Phase settings (thiết kế tái dùng `display_order`, xem "Thứ tự Sub-phase mượn nguyên bộ máy Phase" ở trên) — nhưng màn Signboard không nói ra nên không ai tìm thấy. Nay khi bảng có ≥ 2 nhóm Sub-phase, dưới thanh tìm kiếm hiện một dòng hint nêu đúng luật xếp (khớp Phase → theo vị trí Phase đó; lạ → A→Z sau; "(No sub-phase)" cuối) kèm link mở thẳng `/config/phase`. Một nhóm thì không hiện — không có gì để xếp. Không đổi API/engine; e2e phủ cả hai chiều (hiện khi 2 nhóm, ẩn khi 1 nhóm).
