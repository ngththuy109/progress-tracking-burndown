...
| **Log work** | Team-wide, two views over the same period. **By member**: who is logging work this week (or a chosen range), which open in-charge sub-tasks (Jira Request participants) still have no worklog, a per-project capacity check, and a PM can mark a case verified to stop tracking it. **By PIC / day**: a pivot grid of PIC × day showing hours logged each day. It flags, on a **working day**, logging **≤ 4h** (under), **> 8h** (over), or **nothing at all** on a day already past (missing); and flags any **day off** (weekend/holiday) that has worklog (logged on a day off) — working days come from the reference calendar, so real holidays and make-up days count, not just Sat/Sun. Hover a logged cell to open the tickets logged that day, each linking to Jira. This view spans **every synced Epic** — all projects, any tracking status — and is **not** scoped to the viewer's project permissions |
