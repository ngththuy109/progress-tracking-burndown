...
/**
 * Cửa vào của toàn app khi chạy chế độ OIDC.
 *
 * CHƯA ĐĂNG NHẬP (`/api/me` trả 401 → `useMe` cho `null`) VÀ mode = OIDC thì
 * thay cả app bằng màn hình đăng nhập. Mọi trường hợp khác — mode HEADER, mode
 ...
 * query/mutation và đặt cache `me` về `null` khi mode = OIDC — component này
...
export function AuthGate({ children }: { readonly children: ReactNode }) {
  ...
  if (mode.data?.mode === 'OIDC' && !me.isPending && me.data === null) {
    return <LoginScreen loginUrl={mode.data.loginUrl ?? '/auth/login'} />;
  }
  return <>{children}</>;
}
