...
beforeEach(() => {
  principal = asAdmin();
  app = build([epic('PAY-900', 'ACTIVE'), epic('CRM-7', 'ACTIVE')]);
});
...
describe('kiểm tra khi thêm', () => {
  it('dán 4 key trong đó 1 Task, 1 đã theo dõi, 2 hợp lệ thì trả đúng kết quả từng dòng', async () => {
    const res = await app.inject({
      ...
      payload: { keys: ['PAY-100', 'PAY-7', 'PAY-900', 'PAY-200'] },
    });
    const body = res.json();
  ...
  it('key là Task chứ không phải Epic thì reason = NOT_AN_EPIC', async () => {
    const body = (await app.inject({ method: 'POST', url: '/api/projects/PAY/epics/validate', payload: { keys: ['PAY-7'] } })).json();
  ...
  it('key không tồn tại trên Jira thì reason = NOT_FOUND', async () => {
    const body = (await app.inject({ method: 'POST', url: '/api/projects/PAY/epics/validate', payload: { keys: ['PAY-999'] } })).json();
    expect(body.results[0]).toMatchObject({ valid: false, reason: 'NOT_FOUND' });
  });

  it('Epic CÓ THẬT nhưng của DỰ ÁN KHÁC → NOT_FOUND, kể cả khi tenant kia đang theo dõi nó', async () => {
    // CRM-7 tồn tại trên Jira và đang được tenant CRM theo dõi. Dưới URL của
    // PAY nó phải "không tồn tại" — trả ALREADY_TRACKED hay lý do nào khác là
    // xác nhận cho người ngoài rằng key đó có thật (luật 404-thay-vì-403).
    const body = (await app.inject({ method: 'POST', url: '/api/projects/PAY/epics/validate', payload: { keys: ['CRM-7'] } })).json();
    expect(body.results[0]).toMatchObject({ valid: false, reason: 'NOT_FOUND' });
  });

  it('không có quyền đọc project thì reason = NO_PERMISSION', async () => {
    const body = (await app.inject({ method: 'POST', url: '/api/projects/PAY/epics/validate', payload: { keys: ['HR-1'] } })).json();
  ...
  it('Epic thiếu ngày wbs_* vẫn thêm được nhưng có cảnh báo MISSING_WBS_DATES', async () => {
    const res = await app.inject({ method: 'POST', url: '/api/projects/PAY/epics/validate', payload: { keys: ['PAY-100'] } });
...
describe('vòng đời', () => {
  ...
  it('chuyển PENDING → ACTIVE trực tiếp bị từ chối', async () => {
    ...
    const res = await local.inject({ method: 'PATCH', url: '/api/projects/PAY/epics/PAY-100', payload: { status: 'ACTIVE' } });
  ...
  it('chuyển ACTIVE → PAUSED rồi PAUSED → ACTIVE đều hợp lệ', async () => {
    ...
    expect((await local.inject({ method: 'PATCH', url: '/api/projects/PAY/epics/PAY-100', payload: { status: 'PAUSED' } })).statusCode).toBe(200);
    expect(store.rows.get('PAY-100')!.status).toBe('PAUSED');
    expect((await local.inject({ method: 'PATCH', url: '/api/projects/PAY/epics/PAY-100', payload: { status: 'ACTIVE' } })).statusCode).toBe(200);
  ...
  it('chuyển PAUSED → BACKFILLING bị từ chối', async () => {
    ...
    const res = await local.inject({ method: 'PATCH', url: '/api/projects/PAY/epics/PAY-100', payload: { status: 'BACKFILLING' } });
    expect(res.statusCode).toBe(409);
  });
...
describe('bỏ theo dõi', () => {
  it('xoá với purge=false giữ nguyên dữ liệu lịch sử', async () => {
    ...
    const res = await local.inject({ method: 'DELETE', url: '/api/projects/PAY/epics/PAY-100?purge=false' });
    expect(res.statusCode).toBe(200);
  ...
  it('xoá với purge=true xoá sạch mọi dữ liệu của Epic đó', async () => {
    ...
    const res = await local.inject({
      method: 'DELETE',
      url: '/api/projects/PAY/epics/PAY-100?purge=true',
  ...
  it('purge=true mà KHÔNG gõ lại đúng key thì bị chặn và không xoá gì', async () => {
    const local = build([epic('PAY-100', 'ACTIVE', { historyRows: 120 })]);
    const res = await local.inject({ method: 'DELETE', url: '/api/projects/PAY/epics/PAY-100?purge=true' });
  ...
  it('xoá Epic không có trong sổ trả HTTP 404', async () => {
    const res = await app.inject({ method: 'DELETE', url: '/api/projects/PAY/epics/PAY-404' });
    expect(res.statusCode).toBe(404);
  });

  it('xoá Epic của TENANT KHÁC qua URL dự án mình → 404, không xoá gì', async () => {
    // CRM-7 đang tracked (tenant CRM). PM của PAY không được đụng, và câu trả
    // lời phải giống hệt "không tồn tại".
    const res = await app.inject({ method: 'DELETE', url: '/api/projects/PAY/epics/CRM-7' });
    expect(res.statusCode).toBe(404);
...
describe('danh sách, duyệt và phân quyền', () => {
  it('GET /epics chỉ trả Epic CỦA TENANT trong URL', async () => {
    const local = build([
      epic('PAY-100', 'ACTIVE', { subtaskCount: 25, missingWbsDateCount: 5 }),
      epic('CRM-7', 'ACTIVE'),
    ]);
    const body = (await local.inject({ method: 'GET', url: '/api/projects/PAY/epics' })).json();
    expect(body.epics.map((e: { epicKey: string }) => e.epicKey)).toEqual(['PAY-100']);
    expect(body.epics[0].dataHealth).toMatchObject({ subtaskCount: 25, missingWbsDateCount: 5 });
  });

  it('GET /epics/browse phân trang, KHÔNG cần ?project=, và đánh dấu Epic đã theo dõi', async () => {
    const body = (await app.inject({ method: 'GET', url: '/api/projects/PAY/epics/browse?startAt=0&maxResults=2' })).json();
    expect(body.items).toHaveLength(2);
    expect(body.maxResults).toBe(2);
    expect(body.total).toBeGreaterThan(2);

    const all = (await app.inject({ method: 'GET', url: '/api/projects/PAY/epics/browse?maxResults=100' })).json();
    expect(all.items.find((i: { key: string }) => i.key === 'PAY-900').alreadyTracked).toBe(true);
  ...
  it('PATCH với timezone không hợp lệ bị từ chối HTTP 400', async () => {
    const local = build([epic('PAY-100', 'ACTIVE')]);
    const res = await local.inject({ method: 'PATCH', url: '/api/projects/PAY/epics/PAY-100', payload: { timezone: 'UTC+7' } });
    expect(res.statusCode).toBe(400);
  ...
  it('PATCH với timezone IANA hợp lệ được chấp nhận', async () => {
    const local = build([epic('PAY-100', 'ACTIVE')]);
    const res = await local.inject({ method: 'PATCH', url: '/api/projects/PAY/epics/PAY-100', payload: { timezone: 'Asia/Tokyo' } });
    expect(res.statusCode).toBe(200);
  ...
  it('VIEWER của dự án xem được danh sách nhưng không được thêm/xoá (403)', async () => {
    principal = asViewer('PAY');
    expect((await app.inject({ method: 'GET', url: '/api/projects/PAY/epics' })).statusCode).toBe(200);
    expect((await app.inject(addBody(['PAY-100']))).statusCode).toBe(403);
    expect((await app.inject({ method: 'DELETE', url: '/api/projects/PAY/epics/PAY-900' })).statusCode).toBe(403);
    expect(store.rows.has('PAY-900')).toBe(true);
  });

  it('PM của dự án thêm/xoá được trong dự án của MÌNH', async () => {
    principal = asPm('PAY');
    expect((await app.inject(addBody(['PAY-100']))).statusCode).toBe(200);
    expect(store.rows.has('PAY-100')).toBe(true);
  });

  it('không phải thành viên (PM dự án khác) → 404 PROJECT_NOT_FOUND, kể cả GET', async () => {
    principal = asPm('CRM');
    const res = await app.inject({ method: 'GET', url: '/api/projects/PAY/epics' });
    expect(res.statusCode).toBe(404);
