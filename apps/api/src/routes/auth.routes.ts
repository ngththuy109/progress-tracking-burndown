...
/**
 * Hộp seal/open một BÍ MẬT ngắn (bind password LDAP) — cùng khuôn AES-256-GCM
 * với secret-box của @app/db. Tách interface ở đây để tầng route test được với
 * hộp giả mà không cần khóa thật.
 */
 ...
 *
 * PHÂN QUYỀN (mô hình một tầng của main): nhóm admin tự kiểm principal ngay
 * trong handler theo đúng quy ước của `users.routes.ts` — không có preHandler
 * `requireAdmin` dùng chung.
 */
...
export function registerAuthRoutes(app: FastifyInstance, deps: AuthRouteDeps): void {
  ...
  /**
   * Nhóm admin tự kiểm principal (quy ước một tầng của main): chưa đăng nhập →
   * 401, không phải ADMIN → 403. Trả về principal để handler dùng `userId`.
   */
  const requireAdmin = (req: FastifyRequest): Principal => {
    ...
    if (p.role !== 'ADMIN') {
      throw new ApiError(403, 'FORBIDDEN', 'Only Admins can configure LDAP login.');
    }
    return p;
  };
  ...
  app.get('/api/admin/auth/ldap', async (req, reply) =>
  ...
  app.put('/api/admin/auth/ldap', async (req, reply) =>
    handle(reply, async (): Promise<LdapConfigView> => {
      const adminPrincipal = requireAdmin(req);
  ...
  app.post('/api/admin/auth/ldap/test', async (req, reply) =>
